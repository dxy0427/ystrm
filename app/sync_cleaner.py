    ...
    def _is_source_file_exists(self, relative_path: str) -> bool:
        ...
        if file_ext.lower() == ".strm":
            for video_ext in self.video_exts:
                if os.path.exists(os.path.join(self.library_dir, f"{file_name}{video_ext.lower()}")):
                    return True
            return False
        return os.path.exists(os.path.join(self.library_dir, relative_path))
        ...
        for root, _, files in os.walk(self.dest_dir):
            for file in files:
                ...
                if not os.path.exists(source_file) or \
                   os.path.getmtime(dest_file) > os.path.getmtime(source_file) or \
