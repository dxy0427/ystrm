...
class Config:
    ...
    def _load_config(self):
        ...
        except Exception as e: logger.error(f"配置文件加载失败：{str(e)}", exc_info=True); raise
    def _validate_config(self):
        ...
        for key, typ in required:
            ...
            except (KeyError, TypeError) as e: logger.error(f"配置项错误：{key} - {str(e)}", exc_info=True); raise
        for idx, conf in enumerate(self.config["monitor_confs"]):
            if not all(k in conf for k in ["source_dir", "dest_dir", "library_dir"]):
                logger.error(f"监控配置[{idx}]缺失核心路径"); raise KeyError(f"monitor_confs[{idx}] 核心路径缺失")
    @property
    ...
    def health_check_interval(self) -> int:
        ...
        except (ValueError, TypeError): return 300
    @property
